// pages/admin.tsx
//
// Pages Router variant. Identical UX to app-admin-page.tsx, dùng next/router.
...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (unlocked && password) fetchLeads(password);
...
const S: Record<string, React.CSSProperties> = {
  overlay: { position: 'fixed', inset: 0, background: '#f5f5f7', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' },
  card: { width: '100%', maxWidth: 360, background: '#fff', borderRadius: 12, padding: '28px 24px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', display: 'flex', flexDirection: 'column', gap: 12 },
  cardTitle: { fontSize: 18, fontWeight: 700, margin: '0 0 4px 0' },
  input: { padding: '12px 14px', fontSize: 15, borderRadius: 8, border: '1px solid #d1d1d6', outline: 'none' },
  error: { background: '#fee2e2', color: '#991b1b', padding: '8px 12px', borderRadius: 6, fontSize: 13 },
  errorBanner: { background: '#fee2e2', color: '#991b1b', padding: '12px 16px', borderRadius: 8, marginBottom: 16, maxWidth: 1400, margin: '0 auto 16px' },
  btnPrimary: { padding: '10px 14px', fontSize: 14, fontWeight: 600, color: '#fff', background: '#111', border: 'none', borderRadius: 6, cursor: 'pointer' },
  wrapper: { minHeight: '100vh', background: '#f5f5f7', padding: '24px 20px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', color: '#111' },
  h1: { fontSize: 22, fontWeight: 700, margin: '0 auto 16px', maxWidth: 1400 },
  statsRow: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12, maxWidth: 1400, margin: '0 auto 16px' },
  ...
  filterBar: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', background: '#fff', padding: 10, borderRadius: 10, maxWidth: 1400, margin: '0 auto 16px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' },
  searchInput: { flex: '1 1 220px', padding: '8px 12px', fontSize: 13, border: '1px solid #d1d1d6', borderRadius: 6, outline: 'none' },
